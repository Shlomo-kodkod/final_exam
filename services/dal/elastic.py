...
class Elastic:
    ...
    def index(self, index_name: str, file_id: str, data: dict):
        ...
        """
        try:
            self.__connection.index(index=index_name, id=file_id, document=data,  refresh=True)
    ...
    def search(self, index_name: str, file_id: str) -> list:
        ...
        """
        try:
            result = self.__connection.get(index=index_name, id=file_id, refresh=True)
            if result['found']:
                doc = result['_source']
    ...
    def update_documents(self, index_name: str, id:str, data: dict):
        ...
        """
        try:
            self.__connection.update(index=index_name, id=id, doc=data,  refresh=True)
