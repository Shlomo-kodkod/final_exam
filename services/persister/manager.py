...
class PersisterManager:
    ...
    def process_message(self, message: dict, path_filed: str = "File Path"):
        """
        Process message, indexing in elastic and upload data to MongoDB and publish to kafka message id.
        """
        try:
            ...
            self.__producer.produce(config.KAFKA_AUDIO_TOPIC, config.KAFKA_AUDIO_KEY, id_data)
